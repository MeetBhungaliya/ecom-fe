  ...
  return (
    <div className="flex items-start gap-3 py-2 text-xs">
      ...
      <div className="flex-1 min-w-0">
        ...
        {tx.note && <p className="text-[11px] text-muted-foreground/80 mt-0.5 truncate">{tx.note}</p>}
        ...
        <div className="flex items-center gap-3">
          ...
          {!isDesktop && (
            <div className="flex items-center gap-2">
              {product.imageUrl && (
                <img src={product.imageUrl} alt="" className="h-7 w-7 rounded-md object-cover border border-border" />
              )}
        ...
        <div className="md:col-span-2">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-card border border-border rounded-xl p-5 shadow-sm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <Input id="price" label="Price (₹)" type="number" step="0.01" min="0" error={errors.price?.message} {...register('price')} />
              </div>
              <div>
                <Input id="minimumStock" label="Min Stock Threshold" type="number" min="0" error={errors.minimumStock?.message} {...register('minimumStock')} />
              </div>
              <div className="sm:col-span-2 space-y-1">
                <span className="text-xs font-semibold tracking-wide uppercase text-muted-foreground/80 block">Notes</span>
                <Textarea id="note" placeholder="Notes..." rows={2} className="resize-none min-h-[60px]" {...register('note')} />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-border/60">
              ...
              <Button type="submit" disabled={isPending || !isDirty} className="h-9 px-4 text-xs font-medium">
                {isPending ? (
                  <>
        ...
        <div className="space-y-4">
          <div className="bg-card border border-border rounded-xl p-4 shadow-sm space-y-3">
            <div className="flex items-center justify-between pb-1.5 border-b border-border/60">
              <span className="text-xs font-semibold tracking-wide uppercase text-muted-foreground/80">Stock Details</span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between bg-muted/20 border border-border/30 rounded-lg p-2.5">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] text-muted-foreground uppercase font-medium tracking-wide">In Stock</span>
                  <span
                    className={cn(
              ...
              <div className="bg-muted/30 border border-border/30 rounded-lg p-2.5 flex items-center justify-between">
                <span className="text-[10px] text-muted-foreground uppercase tracking-wide font-medium">Inventory Value</span>
          ...
          <div className="bg-card border border-border rounded-xl p-4 shadow-sm">
            <div className="flex items-center gap-1.5 mb-2 pb-1.5 border-b border-border/60">
              ...
              <span className="text-xs font-semibold tracking-wide uppercase text-muted-foreground/80">Stock History</span>
            </div>
            ...
            {product.recentTransactions && product.recentTransactions.length > 0 ? (
              ...
              <p className="text-[11px] text-muted-foreground text-center py-4">No recent movements</p>
            )}
          </div>
        </div>
      </div>
